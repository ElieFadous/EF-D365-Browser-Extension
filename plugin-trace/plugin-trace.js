/**
 * EF Power Platform Tools — Plugin Trace Logs
 *
 * Full-page viewer for Dynamics 365 Plug-in Trace Log records.
 * Uses a proxy tab (service worker) to fetch data from the target environment
 * via the OData Web API, injecting a content script into the proxy tab.
 */
...
async function _loadIgnoredTypes() {
  const stored = await chrome.storage.local.get('pluginTraceIgnored');
  _ignoredTypes = new Set(Array.isArray(stored.pluginTraceIgnored) ? stored.pluginTraceIgnored : []);
}

async function _saveIgnoredTypes() {
  await chrome.storage.local.set({ pluginTraceIgnored: [..._ignoredTypes] });
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!ENV_URL) {
    _showError('No environment URL provided. Please open this page from the EF Power Platform Tools popup.');
    return;
  }
 ...
 * In that case fetch() won't have InPrivate session cookies — relay through
 * the parent D365 tab via postMessage instead.
 */
const _inModal = window.self !== window.top;

/** Relay a fetch through the modal-shell bridge (InPrivate-safe). */
function _bridgeFetch(url, extraHeaders = {}, method = 'GET', body = null) {
  return new Promise((resolve, reject) => {
    ...
    window.parent.postMessage(
      { __efppt: 'fetch', id, url, method, headers: extraHeaders, body },
      '*'
    );
  });
}
...
/**
 * Fetches a D365 Web API URL.
 * Standalone-tab mode: direct credentialed fetch.
