/**
 * EF Power Platform Tools — Power Automate Flows
 *
 * Full-page viewer for Power Automate (Modern Flow) records and executions.
 * Fetches D365 Web API data directly using credentials:'include' — no proxy tab needed.
 */
...
function _bindControls() {
  ...
  document.getElementById('btn-set-default').addEventListener('click', () => {
    ...
    chrome.storage.local.set({ [DEFAULT_SOL_KEY]: _selectedSolution.solutionid });
...
async function _updateDefaultStar() {
  const stored = await chrome.storage.local.get(DEFAULT_SOL_KEY);
  const btn = document.getElementById('btn-set-default');
  const isDefault = _selectedSolution && stored[DEFAULT_SOL_KEY] === _selectedSolution.solutionid;
