/**
 * ribbon.js — Entry point for the Ribbon Buttons browser.
 *
 * Responsibilities:
 *  • Bootstrap (proxy tab, solutions fetch)
 ...
 *  ribbon-api.js     — D365 Web API calls
...
import { fetchAllPages, sendMessage }    from './ribbon-api.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Step 1: open proxy tab.
  setLoadingText(`Connecting to ${state.envName}…`);
  try {
    const result       = await sendMessage({ type: 'OPEN_PROXY_TAB', env: state.envUrl });
    state.proxyTabId     = result.tabId;
    state.proxyTabReused = result.reused ?? false;
  } catch (e) {
    document.getElementById('env-link').href = state.envUrl;
    console.error('[EF PPT]', e);
    showGlobalError(`Cannot connect to ${state.envName}.\n\n${e.message}\n\nSign in first, then reopen this page.`);
    return;
  }
  ...
  window.addEventListener('beforeunload', () => {
    if (state.proxyTabId !== null) {
      chrome.runtime.sendMessage({
        type:   'CLOSE_PROXY_TAB',
        tabId:  state.proxyTabId,
        reused: state.proxyTabReused,
      }).catch(() => {});
    }
  });

  // Step 2: fetch solution list.
